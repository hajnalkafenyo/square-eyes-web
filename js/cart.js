...
function updateShoppingCartCount() {
  ...
  if (movieListContainer) {
    ...
    movieListContainer.innerHTML = moviesInCart
      .map((m) => movieToHTML(m))
      .join('');
    [...document.getElementsByClassName('remove-from-cart-button')].forEach(
      (v) =>
  ...
  return `
        <section id="movie-details" class="content-box-dark short-movie-details">
            <div class="short-movie-details__poster">
                <img src="${
                  movie.image
                }" id="movie-poster" alt="Movie Poster" />
            </div>
            ...
            <button type="button" data-movie-id="${
              movie.id
            }" class="remove-from-cart-button button">Delete</button>
            </div>
